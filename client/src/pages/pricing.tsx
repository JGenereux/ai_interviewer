...
const PRICE_IDS = {
    ...
    pro: 'price_1SHyPrGRzOLFzWGl8ZKgF8Ny'
};
        ...
        <div className="flex flex-col min-h-screen main-bg">
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
                <motion.div
                    ...
                    className="mb-8"
                >
                    <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#10B981]/20 to-[#3B82F6]/20 border border-[#10B981]/30">
                        <div className="relative flex h-2 w-2">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#10B981] opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2 w-2 bg-[#10B981]"></span>
                        </div>
                        <span className="font-btn-font text-sm text-[#10B981]">1 FREE INTERVIEW</span>
                        <span className="font-nav-font text-neutral-400 text-sm">— Try it before you buy</span>
                    </div>
                </motion.div>
                ...
                >
                    <h1 className="font-header-font text-5xl md:text-6xl text-white mb-4">
                        CHOOSE YOUR PLAN
                    </h1>
                    <p className="font-nav-font text-neutral-400 text-lg max-w-xl mx-auto">
                        Select the plan that fits your interview prep needs
                    </p>
                </motion.div>

                <div className="flex flex-col lg:flex-row items-stretch justify-center gap-6 w-full max-w-6xl mb-12">
                    <PricingCard
                        tier="Starter"
                        price="$20"
                        interviews={6}
...
function PricingCard({
    ...
    return (
        <motion.div
            ...
            className="group relative flex-1 max-w-sm w-full"
        >
            {popular && (
                <div
            ...
            />
            <div
                className={`relative h-full p-8 rounded-2xl bg-[#161616] border transition-all duration-500 flex flex-col ${
                    popular ? 'border-white/20' : 'border-white/10'
            ...
            >
                <div className="flex items-center gap-3 mb-6">
                    ...
                    />
                    <span
                        className="font-btn-font text-sm uppercase tracking-wider"
                        style={{ color: accent }}
                    >
                        {tier}
                    </span>
                ...
                <div className="mb-6">
                    {interviews !== null ? (
                        <>
                            <span className="font-header-font text-5xl text-white">{price}</span>
                            <span className="font-nav-font text-neutral-500 text-sm ml-2">/mo</span>
                        </>
                    ) : (
                        <span className="font-header-font text-3xl text-white">{price}</span>
                    )}
                </div>

                <div className="mb-6">
                    {interviews !== null ? (
                        <p className="font-nav-font text-neutral-300">
                            <span className="text-2xl font-btn-font" style={{ color: accent }}>{interviews}</span>
                            <span className="text-neutral-400 ml-2">interviews included</span>
                        </p>
                    ) : (
                        <p className="font-nav-font text-neutral-400">
                ...
                <div className="flex-1 space-y-3 mb-8">
                    {perks.map((perk, index) => (
                        <div key={index} className="flex items-center gap-3">
                            ...
                            <span className="font-nav-font text-neutral-400 text-sm">{perk}</span>
                        </div>
                    ))}
                </div>

                {interviews !== null && (
                    <p className="font-nav-font text-neutral-500 text-xs text-center mb-3">
                        Monthly subscription
                    </p>
                )}

                <button
                    className="w-full font-btn-font text-sm px-6 py-4 rounded-xl cursor-pointer transition-all duration-300 flex items-center justify-center gap-2"
                    style={{
